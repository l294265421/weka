...
public class StringToWordVector extends Filter implements UnsupervisedFilter, OptionHandler {
	...
	/**
	 * Contains a mapping of valid words to attribute indexes （单词-单词的属性索引）.
	 * 
	 */
	...
	/**
	 * whether to operate on a per-class basis. 值为true时，忽略类属性，所有类的实例被当作一个类的实例对待；
	...
	public Enumeration listOptions() {
		...
		result.addElement(
				new Option("\tConvert all tokens to lowercase before " + "adding to the dictionary.", "L", 0, "-L"));
	...
	public boolean batchFinished() throws Exception {
		...
		if (!isFirstBatchDone()) {
			...
			if (getInputFormat().classIndex() >= 0) {
				if (!getInputFormat().classAttribute().isNominal() || getInputFormat().attributeStats(
						getInputFormat().classIndex()).missingCount == getInputFormat().numInstances()) {
				...
				while (m_Tokenizer.hasMoreElements()) {
					...
					if (index != null) {
						if (m_OutputCounts) { // Separate if here rather than
												...
												// hashtable lookup
							// 词在m_Dictionary中的index怎么能直接用在contained呢？
