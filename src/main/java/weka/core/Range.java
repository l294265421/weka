...
public class Range implements Serializable, RevisionHandler {
	...
	/**
	 * Store the maximum value permitted in the range. -1 indicates that no
	 * upper value has been set
	 */
	...
	/**
	 * Gets whether the range sense is inverted, i.e. all <i>except</i> the
	 * values included by the range string are selected.
	 * 
	...
	/**
	 * Sets whether the range sense is inverted, i.e. all <i>except</i> the
	 * values included by the range string are selected.
	 * 
	 * @param newSetting
	...
	/**
	 * Sets the ranges from a string representation. Note that setUpper() must
	 * be called afterwards for ranges to be actually set internally.
	 *
	 * @param rangeList
	 *            the comma separated list of ranges. The empty string sets the
	 *            range to empty.
	 * @throws IllegalArgumentException
